...
<x-app-layout>
    <div class="flex flex-col justify-center items-start gap-4 w-full">
        <div class="flex flex-col justify-center items-start gap-2">
            <h3 class="text-4xl 2xl:text-5xl font-bold">Kelola Kelas</h3>
            <p class="text-[var(--surface-variant)]">Akses dan kelola Kelas langsung dari sini</p>
        </div>
        <div class="flex justify-end items-center gap-4 w-full">
            ...
            <button type="button" @click="showModalClassroomAdd = true"
        ...
        @foreach ($classrooms as $classroom)
            <div
                ...
                <div class="flex justify-start items-center w-full">
                    <h3 class="text-xl 2xl:text-2xl font-bold text-start">{{ $classroom->name }}</h3>
                </div>
                <div class="flex justify-end items-center gap-2 w-full" x-data="{ showModalClassroom_{{ $classroom->id }}_edit: false }">
                    <button type="button" @click="showModalClassroom_{{ $classroom->id }}_edit = true"
                    ...
                    <form action="{{ route('admin.manage.classrooms.destroy', $classroom->id) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                    ...
                    <div x-show="showModalClassroom_{{ $classroom->id }}_edit && !splash"
                        @click.self="showModalClassroom_{{ $classroom->id }}_edit = false" x-transition.opacity
                        x-init="$el.classList.remove('hidden')"
                        class="hidden fixed inset-0 p-4 !z-[9999] flex justify-center items-center bg-black/50" x-cloak>

                        <form action="{{ route('admin.manage.classrooms.update', $classroom->id) }}" method="POST"
                            @submit="splash = true"
                            ...
                            <div class="flex justify-between items-center gap-2 p-6">
                                <span class="text-2xl">Edit Kelas</span>
                            </div>
                            ...
                            <div class="flex justify-end items-center gap-2 p-6">
                                <button type="button" @click="showModalClassroom_{{ $classroom->id }}_edit = false"
