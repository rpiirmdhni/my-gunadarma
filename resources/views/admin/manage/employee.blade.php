...
<x-app-layout>
    <div class="flex flex-col justify-center items-start gap-4 w-full">
        <div class="flex flex-col justify-center items-start gap-2">
            <h3 class="text-4xl 2xl:text-5xl font-bold">Kelola Pegawai</h3>
            <p class="text-[var(--surface-variant)]">Akses dan kelola Pegawai langsung dari sini</p>
        </div>
        <div class="flex justify-end items-center gap-4 w-full">
            ...
            <button type="button" @click="showModalEmployeeAdd = true"
        ...
        @foreach ($employees as $employee)
            <div
                ...
                <div class="flex flex-col justify-center items-start w-full gap-4">
                    <h3 class="text-xl 2xl:text-2xl font-bold text-start">{{ $employee->fullname }}</h3>
                    <div class="flex flex-col justify-center items-start gap-2">
                        <span
                            class="px-2 py-1 text-sm rounded-md text-white bg-[var(--primary)]">{{ $employee->nip }}</span>
                    </div>
                </div>
                <div class="flex justify-end items-center gap-2 w-full" x-data="{ showModalEmployee_{{ $employee->nip }}_edit: false }">
                    <button type="button" @click="showModalEmployee_{{ $employee->nip }}_edit = true"
                    ...
                    <form action="{{ route('admin.manage.employees.destroy', $employee->nip) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                    ...
                    <div x-show="showModalEmployee_{{ $employee->nip }}_edit && !splash"
                        @click.self="showModalEmployee_{{ $employee->nip }}_edit = false" x-transition.opacity
                        x-init="$el.classList.remove('hidden')"
                        class="hidden fixed inset-0 p-4 !z-[9999] flex justify-center items-center bg-black/50" x-cloak>

                        <form action="{{ route('admin.manage.employees.update', $employee->nip) }}" method="POST"
                            @submit="splash = true"
                            ...
                            <div class="flex justify-between items-center gap-2 p-6">
                                <span class="text-2xl">Edit Pegawai</span>
                            </div>
                            ...
                            <div class="flex justify-end items-center gap-2 p-6">
                                <button type="button" @click="showModalEmployee_{{ $employee->nip }}_edit = false"
