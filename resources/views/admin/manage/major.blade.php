...
<x-app-layout>
    <div class="flex flex-col justify-center items-start gap-4 w-full">
        <div class="flex flex-col justify-center items-start gap-2">
            <h3 class="text-4xl 2xl:text-5xl font-bold">Kelola Jurusan</h3>
            <p class="text-[var(--surface-variant)]">Akses dan kelola Jurusan langsung dari sini</p>
        </div>
        <div class="flex justify-end items-center gap-4 w-full">
            ...
            <button type="button" @click="showModalMajorAdd = true"
        ...
        @foreach ($majors as $major)
            <div
                ...
                <div class="flex justify-start items-center w-full">
                    <h3 class="text-xl 2xl:text-2xl font-bold text-start">{{ $major->name }}</h3>
                </div>
                <div class="flex justify-end items-center gap-2 w-full" x-data="{ showModalMajor_{{ $major->id }}_edit: false }">
                    <button type="button" @click="showModalMajor_{{ $major->id }}_edit = true"
                    ...
                    <form action="{{ route('admin.manage.majors.destroy', $major->id) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                    ...
                    <div x-show="showModalMajor_{{ $major->id }}_edit && !splash"
                        @click.self="showModalMajor_{{ $major->id }}_edit = false" x-transition.opacity
                        x-init="$el.classList.remove('hidden')"
                        class="hidden fixed inset-0 p-4 !z-[9999] flex justify-center items-center bg-black/50" x-cloak>

                        <form action="{{ route('admin.manage.majors.update', $major->id) }}" method="POST"
                            @submit="splash = true"
                            ...
                            <div class="flex justify-between items-center gap-2 p-6">
                                <span class="text-2xl">Edit Jurusan</span>
                            </div>
                            ...
                            <div class="flex justify-end items-center gap-2 p-6">
                                <button type="button" @click="showModalMajor_{{ $major->id }}_edit = false"
