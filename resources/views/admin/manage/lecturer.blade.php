...
<x-app-layout>
    <div class="flex flex-col justify-center items-start gap-4 w-full">
        <div class="flex flex-col justify-center items-start gap-2">
            <h3 class="text-4xl 2xl:text-5xl font-bold">Kelola Dosen</h3>
            <p class="text-[var(--surface-variant)]">Akses dan kelola Dosen langsung dari sini</p>
        </div>
        <div class="flex justify-end items-center gap-4 w-full">
            ...
            <button type="button" @click="showModalLecturerAdd = true"
        ...
        @foreach ($lecturers as $lecturer)
            <div
                ...
                <div class="flex flex-col justify-center items-start w-full gap-4">
                    <h3 class="text-xl 2xl:text-2xl font-bold text-start">{{ $lecturer->fullname }}</h3>
                    <div class="flex flex-col justify-center items-start gap-2">
                        <span
                            class="px-2 py-1 text-sm rounded-md text-white bg-[var(--primary)]">{{ $lecturer->nidn }}</span>
                    </div>
                </div>
                <div class="flex justify-end items-center gap-2 w-full" x-data="{ showModalLecturer_{{ $lecturer->nidn }}_edit: false }">
                    <button type="button" @click="showModalLecturer_{{ $lecturer->nidn }}_edit = true"
                    ...
                    <form action="{{ route('admin.manage.lecturers.destroy', $lecturer->nidn) }}" method="post">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
                    ...
                    <div x-show="showModalLecturer_{{ $lecturer->nidn }}_edit && !splash"
                        @click.self="showModalLecturer_{{ $lecturer->nidn }}_edit = false" x-transition.opacity
                        x-init="$el.classList.remove('hidden')"
                        class="hidden fixed inset-0 p-4 !z-[9999] flex justify-center items-center bg-black/50" x-cloak>

                        <form action="{{ route('admin.manage.lecturers.update', $lecturer->nidn) }}" method="POST"
                            @submit="splash = true"
                            ...
                            <div class="flex justify-between items-center gap-2 p-6">
                                <span class="text-2xl">Edit Dosen</span>
                            </div>
                            ...
                            <div class="flex justify-end items-center gap-2 p-6">
                                <button type="button" @click="showModalLecturer_{{ $lecturer->nidn }}_edit = false"
